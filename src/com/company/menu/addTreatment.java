...
public class addTreatment extends JFrame implements ActionListener{
    ...
    public addTreatment() throws IOException {
        p1=new JPanel();
        p1.setBounds(5,10,480,350);
        p1.setLayout(null);
        p1.setBorder(BorderFactory.createTitledBorder("Treatment"));
        try {
            file = new BufferedWriter(new FileWriter("Treatment_health.txt", true));
        } catch (IOException iOexception) {
            iOexception.printStackTrace();
        }
        l = new DefaultListModel<>();
        l.addElement("");
        File f = new File("D:/akash/Agri_Management_System/Treatment_health.txt");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == cancel) {
            ...
            l.addElement(s);
            try {
                Calf_Management.h.model1.addElement(s);
                file.write(s);
                file.newLine();
                file.flush();
            } catch (Exception exception) {
